...
.Rows(1).VerticalAlighment = x1centre
End With
Next MainWs
...
'Set inital variables for calculations
...
Lastrow = MainWs.Cells(Rows.Count, 1).End(x1Up).Row

'Set inital value of beginning stock value for the first Ticker of MainWs
...
'Colour fill yearly price change: Red for negative, and green for positive change
If (Yearly_Price_Change > 0) Then
MainWs.Range("J" & Summary_Table_Row).Interior.ColourIndex = 4

ElseIf (Yearly_Price_Change <= 0) Then

MainWs.Range("J" & Summary_Table_Row).Interior.ColourIndex = 3
End If

'Print the yearly price as a change as a percent in the Summary Table, Column K
...
'Add i to the summary table row count
Summary_Table_Row = Summary_Table_Row + 1
...
'Else if in the next ticker name, enter new ticker stock volume
Else
Total_Ticker_Volume = Total_Ticker_Volume + MainWs.Cells(i, 7).Value
End If
...
MainWs.Range("O3").Value = "Greteast % Decrease"
MainWs.Range("O4").Value = "Greteast Total volume"
